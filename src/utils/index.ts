...
export function displayPositiveNumber(x: number, noDigits: number = 6, fixedDecimals: number = -1): string {
...
export function displayNumber(x: number, nbrOfDigits: number = 6, fixedDecimals: number = -1): string {
  if (nbrOfDigits < 4) {
  ...
  if (thousandsSeparator != '' && numberOfSeparators > 0) {
    ...
    for (let i = lastSeparator; i > 0; i = i - 3) {
      wholeNumberStr = wholeNumberStr.slice(0, i) + thousandsSeparator + wholeNumberStr.slice(i);
    }
    // console.log("WholeNumberStr: " + wholeNumberStr);
  }
  if (wholeNumberStr.length === nbrOfDigits || wholeNumberStr.length === nbrOfDigits - 1) {
    ...
    if (wholeNumberStr.length < nbrOfDigits) {
      ...
      decimalsStr = decimalsStr ? decimalsStr.substring(0, noDecimals - 1).replace(/0+$/, '') : '';
      if (fixedDecimals >= 0) {
        if (decimalsStr.length > fixedDecimals) {
          decimalsStr = decimalsStr.substring(0, fixedDecimals);
        } else {
          decimalsStr = decimalsStr + '0'.repeat(Math.min(fixedDecimals, noDecimals - 1) - decimalsStr.length);
        }
      }
      if (decimalsStr) {
      ...
      switch (excessRemainder) {
        case 0:
          ...
          break;
        case 1:
          displayStr = displayStr.slice(0, -3) + decimalSeparator + displayStr.slice(-2);
          break;
        case 2:
          displayStr = displayStr.slice(0, -2) + decimalSeparator + displayStr.slice(-1);
          break;
        case 3:
...
export function shortenString(
  ...
  if (!fullStr || fullStr.length <= showStart + showEnd + 2) {
    ...
    return fullStr.slice(0, showStart) + (showEnd > 0 ? seperator + fullStr.slice(-showEnd) : '');
  }
}
...
export function calculateTotalFees(order: any): number {
  const { exchange_fee: exchangeFee, liquidity_fee: liquidityFee, platform_fee: platformFee } = order;
  const totalFees = exchangeFee + liquidityFee + platformFee;
  const decimalPart = (totalFees % 1).toString().split('.')[1];
  return decimalPart && decimalPart.length > 4 ? roundTo(totalFees, 4, RoundType.NEAREST) : totalFees;
}
...
export function calculateAvgFilled(tokenOne: number, tokenTwo: number): number {
  ...
  return decimalPart && decimalPart.length > 8 ? roundTo(avgFilled, 8, RoundType.NEAREST) : avgFilled;
}
...
export function formatNumericString(value: string, separator: string, scale: number): string {
...
export function setLocalStoragePaginationValue(pageSize: number, id?: string) {
  ...
  window.localStorage.setItem(`pagination:${id ?? window.location.pathname}`, String(pageSize));
}

export function getLocalStoragePaginationValue(id?: string) {
  if (typeof window === 'undefined') return undefined;

  const existingValue = window.localStorage.getItem(`pagination:${id ?? window.location.pathname}`);
...
export function searchPairs(query: string, pairsList: any): any {
  ...
  const preprocessPairName = (name: string): string => name.toLowerCase().replace(/\//g, ' ');
  ...
  return pairsList.filter((pair: any) => {
    ...
    const baseMatches = [pairName, pairNameReversed, token1.symbol, token2.symbol, token1.name, token2.name];

    const dynamicMatches = generateCombinations(baseMatches);

    const nameMatches = [...baseMatches, ...dynamicMatches];

    return nameMatches.some((nameMatch) => nameMatch.includes(searchQuery) || hasTypoTolerance(searchQuery, nameMatch));
  });
}
