...
import { WalletDataState, WalletDataStateAccount } from '@radixdlt/radix-dapp-toolkit';
  ...
  reducers: {
    setWalletData: (state: RadixState, action: PayloadAction<WalletData>) => {
      ...
      state.selectedAccount =
        action.payload.accounts.length > 0 ? action.payload.accounts[0] : ({} as WalletDataStateAccount);
    },
    selectAccount: (state: RadixState, action: PayloadAction<WalletDataStateAccount>) => {
